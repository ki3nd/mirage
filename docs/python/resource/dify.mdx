...
Dify documents are mapped to paths using the document's `slug` metadata. If a
document has no `slug` metadata, Mirage falls back to the Dify document name.
...
In the Dify Knowledge UI, open a document and add a metadata item named exactly
`slug`.
...
Use these rules for `slug` values:

- Use `/` to create folders.
- Do not use empty segments, `.`, or `..`.
- Keep each `slug` unique across the dataset.
...
Scoped search uses Dify metadata filtering. Documents with slug metadata are
filtered by `slug`; name-based documents are filtered by `document_name`, which
requires Dify Built-in Fields to be enabled in dataset metadata.
...
| ------------- | -------------------- |
| Document with `slug` metadata | `slug in [...]` |
| Document without `slug` metadata | `document_name in [...]` |
| Folder or glob | `slug` / `document_name` filters for all matched files |

To make scoped search reliable:

1. Add `slug` metadata to every document.
...
If Built-in Fields are disabled, scoped search against documents without `slug`
may return empty results even though `cat`, `grep`, and `find` can still see the
same file.
