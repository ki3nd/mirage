...
### grep and rg match content only

`grep` and `rg` match the memory **text** (the `memory` field), never metadata
such as ids, categories, or timestamps. Because the mount is a directory, plain
`grep` needs `-r` (or a glob); `rg` recurses by default:
...
This is a deliberate divergence: `cat` shows the full JSON, but `grep`/`rg` see
only the memory content, so a search for `food` will not match a memory just
because `food` appears in its `categories`.
...
`cat`, `stat`, `grep`, and `rg` reuse the cached memories until the index
...
| `grep` | Content-only matching over memory text (use `-r` on a directory) |
| `rg` | Content-only matching, recursive by default |
